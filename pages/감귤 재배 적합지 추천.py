...
# 필터링 - 총재배량 > 0 지역만
df_citrus = df_citrus[df_citrus['총재배량(톤)'] > 0]

# 기상 데이터 병합
df_weather_month = df_weather[df_weather['연도'] == selected_year].groupby('지점명').agg({
    '평균기온(°C)': 'mean',
    '평균상대습도(%)': 'mean',
...
df = df_weather_month.merge(df_citrus[['읍면동', '총재배량(톤)']], on='읍면동', how='inner')
