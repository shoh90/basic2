...
# 읍면동 기준 테이블 생성
df_base = df_coords.merge(df_citrus[df_citrus['연도'] == selected_year][['읍면동', '총재배량(톤)']], on='읍면동', how='left')
...
# 적합도 계산
df['기온적합'] = df['평균기온(°C)'].apply(lambda x: 1 if pd.notnull(x) and 15 <= x <= 25 else 0)
df['습도적합'] = df['평균상대습도(%)'].apply(lambda x: 1 if pd.notnull(x) and 60 <= x <= 80 else 0)
df['강수적합'] = df['월합강수량(00~24h만)(mm)'].apply(lambda x: 1 if pd.notnull(x) and 30 <= x <= 100 else 0)
df['풍속적합'] = df['평균풍속(m/s)'].apply(lambda x: 1 if pd.notnull(x) and x <= 4 else 0)
df['일조적합'] = df['합계 일조시간(hr)'].apply(lambda x: 1 if pd.notnull(x) and x >= 150 else 0)

df['적합도점수'] = df[['기온적합', '습도적합', '강수적합', '풍속적합', '일조적합']].sum(axis=1)
df['결과'] = df['적합도점수'].apply(lambda x: '적합' if x >= 4 else '부적합')
