...
#- **매핑 방식:** 읍면동을 주요 기상 관측소에 수동으로 매핑하여 사용합니다. (`mapping` 딕셔너리 참고)
#- **적합도 기준 (월별):** 제시된 기준은 일반적인 예시이며, 실제 감귤 생육 단계 및 품종에 따라 달라질 수 있습니다. **특히 "적합" 지역이 나타나지 않는다면, 이 기준값들이 너무 엄격하거나 해당 월의 기후 조건과 맞지 않을 수 있으므로 조정이 필요합니다.**
- **총재배량(지도 마커 크기):** 지도 상의 원 크기는 선택된 연도의 **연간 총재배량**을 나타냅니다. 월별 분석과는 직접적인 관련이 적을 수 있습니다.
""")
