...
# 🔎 [디버그용] 데이터 로드 결과 확인
st.subheader("📊 데이터 로드 결과 확인 (디버그)")
st.write("pest_disease_4 데이터", df_pest_4.shape, df_pest_4.head())
st.write("pest_disease_5 데이터", df_pest_5.shape, df_pest_5.head())
st.write("asos_weather 데이터", df_weather.shape, df_weather.head())
...
if not df_weather.empty and '지점명' in df_weather.columns and '평균기온(°C)' in df_weather.columns:
    ...
    st.warning("asos_weather 데이터에 지점명 또는 평균기온 정보가 없습니다.")
